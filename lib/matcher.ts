...
const roleAliases: Record<string, string[]> = {
  ...
  fp:["fp&a","fpa","planning","forecasting","financial planning"], fpa:["fp&a","fpa","planning","forecasting","financial planning"],
  accounting:["accounting","accountant","accounts"], ap:["accounts payable","payables"], ar:["accounts receivable","receivables"],
...
function containsTerm(haystack:string,term:string){const n=normalize(term);return !!n&&` ${haystack} `.includes(` ${n} `);}
function aliasesFor(requested:string){const n=normalize(requested);return roleAliases[n]??[n];}
function roleMatch(role:string,title:string,description:string){
  const normalizedRole=normalize(role),requested=tokens(normalizedRole),titleText=normalize(title),descriptionText=normalize(description);
  ...
  return {titleHits,descriptionHits,total:Math.max(requested.length,1),exactTitle,phraseHits};
}
function skillMatch(skills:string[],job:Job){const haystack=normalize([job.title,job.description,...job.skills].join(" "));return skills.filter(skill=>{const n=normalize(skill);return !!n&&(containsTerm(haystack,n)||tokens(skill).some(t=>t.length>2&&containsTerm(haystack,t)));});}
export function isRoleRelevant(job:Job,profile:SearchFilters){const role=roleMatch(profile.role,job.title,job.description);return role.phraseHits||role.exactTitle||role.titleHits.length>0;}
export function matchJob(job:Job,profile:SearchFilters):MatchResult{
  ...
  const roleScore=Math.min(40,(role.titleHits.length/role.total)*32+(role.descriptionHits.length/role.total)*8);
