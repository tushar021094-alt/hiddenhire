...
const COUNTRY_PATTERNS: Array<[string, RegExp]> = [
  ["India", /bindiab|bindianb/i],
  ["United States", /busab|bu.s.a?.?b|bunited statesb|bnew yorkb|bcaliforniab|btexasb/i],
  ["Canada", /bcanadab|btorontob|bvancouverb|bmontrealb/i],
  ["United Kingdom", /bukb|bunited kingdomb|blondonb|benglandb/i],
  ["Australia", /baustraliab|bsydneyb|bmelbourneb/i],
  ["Singapore", /bsingaporeb/i],
  ["UAE", /buaeb|bunited arab emiratesb|bdubaib|babudhabib/i],
  ["Germany", /bgermanyb|bberlinb|bmunichb/i],
];
...
function indiaEligibility(location: string, description: string, country?: string, remote = false) {
  ...
  if (/bindiab|bbengalurub|bbangaloreb|bdelhib|bmumbaib|bhyderabadb|bpuneb|bnoidab|bgurgaonb|bgurugramb/.test(loc)) return true;
  return remote && /remote.{0,30}(india|apac|asia)|(?:india|apac|asia).{0,30}remote/i.test(description);
}

function normalizeCompany(board: string) { return board.replace(/[-_]/g, " ").replace(/bw/g, c => c.toUpperCase()); }
...
export async function fetchGreenhouseBoard(board: string): Promise<Job[]> {
  try {
    ...
    return (data.jobs ?? []).map(job => {
      ...
      const remote = /^remoteb/i.test(location) || /bremotes*(?:-)?s*(?:india|apac|asia|worldwide|global)/i.test(location);
